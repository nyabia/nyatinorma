...
export default async function(pi:ExtensionAPI) {
  ...
  pi.on('tool_call',async(event,ctx)=>{
    if(currentRun()?.status==='blocked'&&(event.toolName==='ny_run'&&event.input.operation!=='list'||['ny_drag','ny_run_select','ny_execute_flow','ny_wait','ny_locate'].includes(event.toolName))){
      ctx.abort();return {block:true,reason:'run_blocked: 사용자가 /play [교정 내용]으로 재개해야 합니다. 먼저 중단 사유를 설명하세요.'};
    }
    if(['ny_drag','ny_run_select','ny_execute_flow','ny_wait','ny_locate'].includes(event.toolName)){
...
For each NEW visual click target, PREFER ny_locate(target, snapshotId) to determine the point, even when you recognize the button. Recognizing the target is not the same as having grounded coordinates. Do not spend planner reasoning estimating pixel positions, converting pixels to fractions, adding crop offsets, or manually walking a sequence of grid crops. ny_locate delegates zoom, grid selection, small crosshair moves and confirmation to one-token non-thinking SELECT requests; it is designed to reduce those expensive planner turns and coordinate reasoning. Reuse the screenshot already returned by the previous tool. Supply a short visual target and only relevant local constraints. Optionally start at an obvious regionPath; if unsure, omit it and let the tool search the full window. Normally omit maxSteps to use 20 calls. When this click is already within the user's requested scope and you have a static anchor and expected outcome, use click:true to locate and act together. Otherwise obtain coordinates with click:false, the default. Do not call ny_preview again routinely after ny_locate has confirmed the point.
Direct coordinate clicking (ny_act) is unavailable. For a one-off click use ny_locate with click:true; do not simulate a click with a zero-length drag or create a throwaway set/flow to bypass localization. For reusable click definitions, first ground each new target with ny_locate(click:false) and reuse its returned point exactly. For drag endpoints, anchors and reusable definitions, regionPath/gridPoint/toGridPoint let the runtime transform cell-local coordinates; do not do that arithmetic yourself. Use ny_observe crops for reading small text/status or clarifying target identity, not as a manual replacement for ny_locate's position search. If ny_locate cannot locate the target, inspect its reason/evidence and revise the target or starting area; do not manufacture a coordinate to bypass failed confirmation. Never infer offscreen content or list boundaries.
Before acting, inspect available reusable sets for this scenario. Use ny_run_select for a matching validated set or recurring choice screen; define a set when repeated use will repay setup. Do not replace an existing suitable SELECT set with repeated localization calls. For different first-seen screens use ny_locate to ground new click targets and ny_drag for observed drags. One-off clicks through ny_locate(click:true) and drags through ny_drag require an observed target, a small distinct STATIC screen anchor, and an expected visible outcome, and return a fresh image. The runtime states the configured drag adapter. Every successful action tool already returns a fresh screenshot. Use that returned image directly for the next decision; do not call ny_observe again just to get a fresh image. Observe again only for a crop, ambiguity, elapsed waiting, or an explicit stale-image error. One action per observation cycle; don't queue actions from an old screenshot. WAIT/THINK are automatic. SELECT uses the currently selected pi model without thinking; ny_reasoning controls only planner reasoning.
For repetitive work use ny_define_flow then ny_execute_flow: a single observed state is enough. The runtime selects action/DONE/WAIT/REPLAN without planner round trips, including repeated drags. Define a visible doneWhen, a progressRegion and grounded actions with when conditions. For list exploration set memoryMode=scan to remember previously seen regions; recurrence is not proof of the end. For a scroll region use drag targetGuard=region; moving list contents are progress, not a stale target. Keep anchors outside the moving contents. For search, stop when a matching item is visible; do not scroll blindly to the end. Unknown screens return for planning. Never interpret no movement alone as completion. On repeated direct actions, create a reusable flow instead of continuing ny_drag/ny_locate one at a time. Link next states only after observing them. If a flow returns local_goal_observed, inspect the evidence; this does not mark the entire task complete. Save useful flows through ny_flow publish; no preset is required.
