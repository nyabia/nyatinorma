...
export default async function(pi:ExtensionAPI) {
  ...
  pi.on('tool_call',async(event,ctx)=>{
    if(currentRun()?.status==='blocked'&&(event.toolName==='ny_run'&&event.input.operation!=='list'||['ny_act','ny_drag','ny_run_select','ny_execute_flow','ny_wait'].includes(event.toolName))){
      ctx.abort();return {block:true,reason:'run_blocked: 사용자가 /play [교정 내용]으로 재개해야 합니다. 먼저 중단 사유를 설명하세요.'};
    }
    if(['ny_act','ny_drag','ny_run_select','ny_execute_flow','ny_wait'].includes(event.toolName)){
